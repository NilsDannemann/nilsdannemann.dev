...
function stag_posted_on() {
	...
	printf( __( '<span class="posted-on">%1$s</span><span class="reading-time">%3$s</span><span class="byline">by %2$s</span>', 'stag' ),
...
function stag_archive_title() {
	if ( is_category() ) :
		printf( __( 'All Posts Under %s', 'stag' ), '<span>' . single_cat_title( '', false ) . '</span>' );

	elseif ( is_tag() ) :
		printf( __( 'All Posts Under %s', 'stag' ), '<span>' . single_tag_title( '', false ) . '</span>' );
...
function stag_list_authors() {
	...
	if ( 1 === $authors_count ) {
		$class = 'one-column';
	} elseif ( 2 === $authors_count ) {
		$class = 'two-column';
	} elseif ( 3 === $authors_count ) {
		$class = 'three-column';
	} elseif ( 4 === $authors_count || $authors_count > 4 ) {
		$class = 'four-column';
	}
...
if ( ! function_exists( 'ink_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 *
 * @since 1.3.0.
 */
function ink_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous nav-link">%link</div>', _x( '<span class="meta-nav"><span class="arrow">←</span> Previous Post</span>', 'Previous post link', 'stag' ) );
				next_post_link( '<div class="nav-next nav-link">%link</div>', _x( '<span class="meta-nav">Next Post <span class="arrow">→</span>', 'Next post link', 'stag' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
if ( ! function_exists( 'ink_maybe_show_archive_title' ) ) :
/**
 * Show Archive page title in certain cases.
 *
 * @param string $region Page Context.
 * @return void
 */
function ink_maybe_show_archive_title( $region ) {
	if ( ! in_array( $region, array( 'inner', 'outer' ) ) ) {
		return;
	}

	$nav_layout = stag_theme_mod( 'layout_options', 'nav-layout' );

	if ( 'traditional' === $nav_layout && 'outer' === $region ) {
		get_template_part( 'partials/archive', 'outer-layout' );
	} elseif ( 'sidebar' === $nav_layout && 'inner' === $region ) {
		get_template_part( 'partials/archive', 'inner-layout' );
	}
