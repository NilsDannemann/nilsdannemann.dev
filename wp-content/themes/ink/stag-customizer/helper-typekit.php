...
class Stag_Typekit_Customizer {
		...
		$wp_customize->add_control(
			new Stag_Customize_Misc_Control(
				...
				array(
					'section'     => $section,
					'type'        => 'text',
					'description' => sprintf( __( 'For more information about Typekit integration, please see <a class="external-link" target="_blank" href="%1$s">%2$s\' documentation</a>.', 'stag' ), 'http://docs.codestag.com/article/144-typekit-integration', STAG_THEME_NAME ),
					'priority'    => 182,
				)
			)
		);
	...
	public function customize_save_after() {
		$typekit_id_has_changed                      = ( get_theme_mod( 'typekit-id' ) !== $this->typekit_id_before_save );
		$typekit_id_has_not_changed_but_choices_have = ( get_theme_mod( 'typekit-id' ) === $this->typekit_id_before_save && get_theme_mod( 'typekit-choices' ) !== get_theme_mod( 'typekit-temp-choices' ) );
		...
		} elseif ( true === $typekit_id_has_changed || true === $typekit_id_has_not_changed_but_choices_have ) {
